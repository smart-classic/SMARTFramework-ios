 ...
 *
 *  Makes a call to /records/{record_id}/imaging_studies/, originally named "get_imaging_studies".
 *  @param callback A SMSuccessRetvalueBlock block that will have a success flag and a user info dictionary containing
 *  the desired objects (key: SMARTResponseArrayKey) if successful.
 */
- (void)getImagingStudies:(SMSuccessRetvalueBlock)callback
{
	NSString *path = [NSString stringWithFormat:@"/records/%@/imaging_studies/", self.record_id];
	[self getObjectsOfClass:[SMImagingStudy class] from:path callback:callback];
}

/**
 *  Returns data about all the available medical images of the patient's body..
 *
 *  Makes a call to /records/{record_id}/medical_images/, originally named "get_medical_images".
 *  @param callback A SMSuccessRetvalueBlock block that will have a success flag and a user info dictionary containing
 *  the desired objects (key: SMARTResponseArrayKey) if successful.
 */
- (void)getMedicalImages:(SMSuccessRetvalueBlock)callback
{
	NSString *path = [NSString stringWithFormat:@"/records/%@/medical_images/", self.record_id];
	[self getObjectsOfClass:[SMMedicalImage class] from:path callback:callback];
}
