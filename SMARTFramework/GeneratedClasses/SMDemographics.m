...
- (SMValueAndUnit *)gestationalAgeAtBirth
{
	if (!_gestationalAgeAtBirth) {
		...
		self.gestationalAgeAtBirth = obj ? obj : (id)[NSNull null];
	}
	
	// we use NSNull as a placeholder in case we already searched the graph and haven't found the object. This should help with performance.
	else if ((id)[NSNull null] == _gestationalAgeAtBirth) {
		return nil;
	}
...
- (SMName *)n
{
	if (!_n) {
		...
		// create an item for this object
		SMName *obj = [SMName newWithSubject:rslt.object inModel:self.model];
		self.n = obj ? obj : (id)[NSNull null];
	}
